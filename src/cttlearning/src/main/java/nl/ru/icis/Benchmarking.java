...
import nl.ru.icis.oracle.YannakakisCTT;
...
public class Benchmarking {
    ...
    public static final String[] eqMethodsAvailable = {"w", "wp", "soucha_hsi", "soucha_h", "soucha_spy", "soucha_spyh", "hybrid_ads"};
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            int extra_states = 0;
            if (line.hasOption(EXTRA_STATES))
                extra_states = Integer.valueOf(line.getOptionValue(EXTRA_STATES).toString());
...
//			boolean isEquiv = mealyss.getStates().size()==finalHyp.getStates().size();
    ...
    private static EquivalenceOracle<MealyMachine<?, String, ?, String>, String, Word<String>> buildEqOracle(
        ...
        switch (ctt_name) {
            ...
            case "hybrid_ads":
                try {
                    eqOracle = new YannakakisCTT(extra_states, oracleForEQoracle);
                } catch (IOException e) {
                    e.printStackTrace();
                }
                logger.logEvent("EquivalenceOracle: HybridADS(" + extra_states + ")");
            case "w":
